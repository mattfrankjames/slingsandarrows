 ...
 */
export default [
  // Build output only. `.netlify/` is generated by `netlify dev`, which bundles
  // every function into .netlify/functions-serve/ — 24k lint errors' worth of
  // other people's transpiled code, appearing the first time anyone runs the
  // local database. ESLint's flat config does not read .gitignore, so being
  // ignored by git is not enough.
  {
    ignores: [
      'dist/**',
      '.eleventy/**',
